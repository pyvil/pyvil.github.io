...
Helper.prototype = {
     ...
     * @param type
     */
    centerOverlayInsideBox : function(obj, box, type) {
        type = ( (type == 'top') || (type == 'left') ) ? type : 'both';

        switch (type) {
            case 'top' :
                $(obj).css("top", Math.max(0, (($(box).height() - $(obj).outerHeight()) / 2)) + "px");
                break;

            case 'left' :
                $(obj).css("left", Math.max(0, (($(box).width() - $(obj).outerWidth()) / 2)) + "px");
                break;
...
var Bday = (function () {
    ...
    this.prepare = function () {
        $('.content').css('width', $('.slide').width() * $('.slide').size())
        ...
        this.page = $('.slide:eq(0)');
    };

    var sliding = function (direction) {
        /*switch  (direction) {
